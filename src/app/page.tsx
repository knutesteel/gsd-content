...
const statusOrder: ContentStatus[] = ["new", "auto_added", "generated", "images_generated", "posted", "archived"];
const labels: Record<ContentStatus, string> = { new: "New", auto_added: "Auto-Added", generated: "Prompt Generated", images_generated: "Images Generated", posted: "Posted", archived: "Archived" };

function identifierParts(value: string) { const [root, variation = "0"] = value.split("-"); return [Number(root) || 0, Number(variation) || 0]; }
function compareNullable(a: string | number | boolean | null | undefined, b: string | number | boolean | null | undefined) {
  if (typeof a === "number" || typeof b === "number") return Number(a ?? -1) - Number(b ?? -1);
...
function sortItems(items: ContentItem[], sort: string, direction: string) {
  ...
  return items.toSorted((a, b) => {
    if (sort === "identifier") { const [ar,av] = identifierParts(a.identifier); const [br,bv] = identifierParts(b.identifier); return (ar - br || av - bv) * factor; }
    const fields: Record<string, keyof ContentItem> = {
      title: "title", status: "status", type: "content_type", panels: "panel_count", overview: "overview",
      content: "content", caption: "caption", first_comment: "first_comment", generation_prompt: "generation_prompt",
      score: "score", priority: "priority", favorite: "is_favorite", instagram_url: "instagram_url",
      publishing_notes: "publishing_notes", generated: "generated_at", posted: "posted_at", archived: "archived_at",
      version: "record_version", created: "created_at", updated: "updated_at",
    };
    const field = fields[sort] ?? "created_at";
    return compareNullable(a[field] as string | number | boolean | null, b[field] as string | number | boolean | null) * factor;
  });
}
...
export default async function Home({ searchParams }: { searchParams: Promise<{ status?: string; sort?: string; direction?: string; q?: string; favorite?: string; notice?: string }> }) {
  ...
  const statusFilterHref = (status: ContentStatus) => {
    const next = new URLSearchParams();
    if (params.q) next.set("q", params.q);
    if (params.sort) next.set("sort", params.sort);
    if (params.direction) next.set("direction", params.direction);
    if (params.favorite === "1") next.set("favorite", "1");
    if (status !== selectedStatus) next.set("status", status);
  ...
  return <main className="app-shell">
    ...
    <section className="status-grid">{statusOrder.map((status) => <Link key={status} href={statusFilterHref(status)} data-active={status === selectedStatus} aria-current={status === selectedStatus ? "page" : undefined}><span>{labels[status]}</span><strong>{counts[status]}</strong></Link>)}</section>
    <AutoFilterForm className="filters"><input name="q" defaultValue={params.q} placeholder="Search identifiers, captions, sources…" aria-label="Search content" /><select name="status" defaultValue={params.status ?? ""}><option value="">All Statuses</option>{statusOrder.map((s) => <option key={s} value={s}>{labels[s]}</option>)}</select><select name="sort" defaultValue={params.sort ?? "created"}><option value="identifier">Identifier</option><option value="title">Title</option><option value="status">Status</option><option value="type">Type</option><option value="panels">Panels</option><option value="overview">Overview</option><option value="content">Content</option><option value="caption">Caption</option><option value="first_comment">First Comment</option><option value="generation_prompt">Generation Prompt</option><option value="score">Score</option><option value="priority">Priority</option><option value="favorite">Favorite</option><option value="instagram_url">Instagram URL</option><option value="publishing_notes">Publishing Notes</option><option value="generated">Prompt Generated Date</option><option value="posted">Posted Date</option><option value="archived">Archived Date</option><option value="version">Version</option><option value="created">Date Added</option><option value="updated">Updated Date</option></select><select name="direction" defaultValue={params.direction ?? "desc"}><option value="desc">Descending</option><option value="asc">Ascending</option></select><label className="check"><input type="checkbox" name="favorite" value="1" defaultChecked={params.favorite === "1"} /> Favorites</label><Link href="/">Clear</Link></AutoFilterForm>
    <section className="dashboard-groups" aria-label="Content items">{groupedItems.map((group) => <details className="content-group" key={group.status} open><summary className="group-heading"><h2>{labels[group.status]}</h2><span>{group.items.length}</span></summary><div className="content-list"><div className="list-head"><span>Identifier / Title</span><span>Type</span><span>Panels</span><span>Score</span><span>Updated</span><span>Actions</span></div>{group.items.map((item) => <article className="content-row" key={item.id}><Link className="content-main" href={`/content/${item.id}`}><div className="identifier">#{item.identifier}{item.is_favorite ? <span aria-label="Favorite">★</span> : null}</div><div><h2>{item.title || "Untitled Idea"}</h2><p>{item.overview || item.caption || item.content || "No content added yet."}</p>{item.status !== "posted" && pendingHandoffs.has(item.id) ? <small>Posted?</small> : null}</div>{thumbnailsByItem.get(item.id)?.length ? <div className="dashboard-thumbnails" aria-label={`Generated images for ${item.title || `item ${item.identifier}`}`}>{thumbnailsByItem.get(item.id)?.map((thumbnail, index) => <Image key={thumbnail.id} src={thumbnail.url} alt={`${item.title || `Item ${item.identifier}`} generated image ${thumbnail.slide ?? index + 1}`} width={64} height={64} unoptimized />)}</div> : null}</Link><span>{item.content_type || "—"}</span><span>{item.panel_count ?? "—"}</span><strong className="score-value">{item.score ?? "—"}</strong><time>{new Date(item.status === "posted" && item.posted_at ? item.posted_at : item.updated_at).toLocaleDateString()}</time><div className="row-actions"><Link href={`/content/${item.id}`}>Open</Link>{item.status !== "archived" ? <form action={quickStatus}><input type="hidden" name="id" value={item.id} /><input type="hidden" name="status" value="archived" /><button aria-label={`Archive ${item.identifier}`}>Archive</button></form> : <form action={quickStatus}><input type="hidden" name="id" value={item.id} /><input type="hidden" name="status" value="new" /><input type="hidden" name="restore" value="1" /><button>Restore</button></form>}</div></article>)}</div></details>)}{items.length === 0 ? <div className="empty">No records match these filters.</div> : null}</section>
  </main>;
}
