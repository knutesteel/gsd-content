...
import { createDmTemplate, markCreatorMessagesRead, recordCreatorDmSent, updateCreatorFollowState, updateCreatorStatus } from "./actions";
  ...
  return (
    <section className="collab-page">
      ...
      <div className="collab-table-wrap">
        ...
        {visible.map((creator) => (
          <article className="collab-table collab-row" key={creator.id} role="button" tabIndex={0} onClick={() => setSelected(creator)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setSelected(creator); }}>
            <div className="collab-creator"><span className="collab-rank">{creator.rank}</span><div><h2>{creator.creator_name}</h2><a href={`https://www.instagram.com/${creator.instagram_handle.replace(/^@/, "")}/`} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()}>{creator.instagram_handle || "No handle listed"}</a><p>{creator.description}</p><small>{creator.fit_rationale}</small></div></div>
