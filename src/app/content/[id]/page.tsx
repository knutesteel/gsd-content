...
import { duplicateItem, generateContent, importLegacyImages, quickStatus, recordGeneration } from "@/app/actions";
...
export default async function ContentDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ notice?: string }> }) {
  ...
  const [{ data: sources }, { data: activeItems }] = await Promise.all([
    sourceIds.length ? supabase.from("sources").select("*").in("id", sourceIds) : Promise.resolve({ data: [] }),
    supabase.from("content_items").select("id").order("created_at", { ascending: false }),
  ]);
  const currentIndex = (activeItems ?? []).findIndex((candidate) => candidate.id === item.id);
  const previousItemId = currentIndex > 0 ? activeItems?.[currentIndex - 1]?.id ?? null : null;
  const nextItemId = currentIndex >= 0 ? activeItems?.[currentIndex + 1]?.id ?? null : null;
  const archiveReturnPath = nextItemId ? `/content/${nextItemId}` : previousItemId ? `/content/${previousItemId}` : "/";
  ...
  const attachedImages = signedAssets.filter((asset) => asset.kind === "image" || asset.kind === "carousel_slide" || asset.kind === "thumbnail");
  const canImportLegacyImages = item.status === "generated" && (item.panel_count ?? 0) > 0 && attachedImages.length === 0;
  return <main className="app-shell">
    <header className="topbar"><Link className="brand" href="/">GSD Content</Link><Link href="/">← Dashboard</Link></header>
    <section className="detail-head"><div><p className="eyebrow">Content {item.identifier}</p><h1>{item.title || "Untitled Idea"}</h1><p>Version {item.record_version} · Updated {new Date(item.updated_at).toLocaleString()}</p></div><div className="button-row detail-navigation">{previousItemId ? <Link href={`/content/${previousItemId}`}>← Previous</Link> : <span className="nav-disabled">← Previous</span>}{nextItemId ? <Link href={`/content/${nextItemId}`}>Next →</Link> : <span className="nav-disabled">Next →</span>}<form action={duplicateItem}><input type="hidden" name="id" value={item.id} /><button>Duplicate Idea</button></form>{item.status !== "archived" ? <form action={quickStatus}><input type="hidden" name="id" value={item.id} /><input type="hidden" name="status" value="archived" /><input type="hidden" name="return_to" value={archiveReturnPath} /><button className="danger">Archive</button></form> : <form action={quickStatus}><input type="hidden" name="id" value={item.id} /><input type="hidden" name="status" value="new" /><input type="hidden" name="restore" value="1" /><button>Restore</button></form>}<DeleteButton id={item.id} identifier={item.identifier} /></div></section>
    {notice ? <div className="notice">{notice}</div> : null}
    {signedAssets.length ? <section className="panel"><h2>Assets ({signedAssets.length})</h2><AssetLightbox identifier={item.identifier} assets={signedAssets.flatMap((asset) => asset.url ? [{ id: asset.id, url: asset.url, alt: `Content ${item.identifier}${asset.slide_number ? ` slide ${asset.slide_number}` : " image"}`, label: asset.slide_number ? `Slide ${asset.slide_number}` : asset.kind, storagePath: asset.storage_path }] : [])} /></section> : null}
    <section className="panel"><div className="panel-title"><h2>Content Tools</h2><div className="button-row"><form action={generateContent}><input type="hidden" name="id" value={item.id} /><GenerateContentButton /></form><GenerateImagesButton id={item.id} identifier={item.identifier} prompt={item.generation_prompt ?? ""} />{canImportLegacyImages ? <form action={importLegacyImages}><input type="hidden" name="id" value={item.id} /><input type="hidden" name="identifier" value={item.identifier} /><button className="primary">Import Images</button></form> : null}<CopyButton value={item.generation_prompt ?? ""} label="Copy Prompt" /><CopyButton value={item.caption ?? ""} label="Copy Caption" /><CopyButton value={item.first_comment ?? ""} label="Copy First Comment" /><form action={recordGeneration}><input type="hidden" name="id" value={item.id} /><input type="hidden" name="prompt" value={item.generation_prompt ?? ""} /><button disabled={!item.generation_prompt}>Record Manual Generation</button></form><PublishingButton id={item.id} caption={item.caption ?? ""} />{item.status !== "posted" ? <form action={quickStatus}><input type="hidden" name="id" value={item.id} /><input type="hidden" name="status" value="posted" /><button className="primary">Mark Posted</button></form> : null}</div></div>{hasPendingHandoff ? <p className="notice">Publishing started but this item is not marked Posted yet.</p> : null}</section>
