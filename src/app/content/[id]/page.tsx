...
export default async function ContentDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ notice?: string }> }) {
  ...
  return <main className="app-shell">
    ...
    {signedAssets.length ? <section className="panel"><h2>Assets ({signedAssets.length})</h2><AssetLightbox assets={signedAssets.flatMap((asset) => asset.url ? [{ id: asset.id, url: asset.url, alt: `Content ${item.identifier}${asset.slide_number ? ` slide ${asset.slide_number}` : " image"}`, label: asset.slide_number ? `Slide ${asset.slide_number}` : asset.kind }] : [])} /></section> : null}
