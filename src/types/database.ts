...
export type Database = {
  public: {
    Tables: {
      ...
      activity_events: { Row: { id: number; owner_id: string; content_item_id: string | null; event_type: string; details: Json; created_at: string }; Insert: never; Update: never; Relationships: [] };
      generation_runs: { Row: { id: string; owner_id: string; content_item_id: string; idempotency_key: string; status: "queued" | "running" | "succeeded" | "failed" | "cancelled"; prompt: string; output: Json | null; error_message: string | null; promoted_at: string | null; started_at: string | null; completed_at: string | null; created_at: string }; Insert: never; Update: never; Relationships: [] };
    };
