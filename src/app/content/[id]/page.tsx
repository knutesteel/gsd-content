...
export default async function ContentDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ notice?: string }> }) {
  ...
  return <main className="app-shell">
    ...
    <section className="panel"><div className="panel-title"><h2>Content Tools</h2><div className="button-row"><CopyButton value={item.caption ?? ""} label="Copy Caption" /><CopyButton value={item.generation_prompt ?? ""} label="Copy Prompt" /><form action={generateContent}><input type="hidden" name="id" value={item.id} /><input type="hidden" name="record_version" value={item.record_version}/><input type="hidden" name="prompt" value={item.generation_prompt ?? ""}/><button className="primary" disabled={!item.generation_prompt}>Generate with AI</button></form><form action={recordGeneration}><input type="hidden" name="id" value={item.id} /><input type="hidden" name="prompt" value={item.generation_prompt ?? ""} /><button disabled={!item.generation_prompt}>Record Manual Generation</button></form>{item.status !== "posted" ? <form action={quickStatus}><input type="hidden" name="id" value={item.id} /><input type="hidden" name="status" value="posted" /><button className="primary">Mark Posted</button></form> : null}</div></div></section>
