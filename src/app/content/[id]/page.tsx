...
export default async function ContentDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ notice?: string }> }) {
  ...
  const [{ data: sources }, { data: activeItems }] = await Promise.all([
    ...
    supabase.from("content_items").select("id").neq("status", "archived").order("created_at", { ascending: false }),
  ]);
  const currentIndex = (activeItems ?? []).findIndex((candidate) => candidate.id === item.id);
  const nextItemId = currentIndex >= 0 ? activeItems?.[currentIndex + 1]?.id ?? activeItems?.[0]?.id : activeItems?.[0]?.id;
  const archiveReturnPath = nextItemId && nextItemId !== item.id ? `/content/${nextItemId}` : "/";
  ...
  const attachedImages = signedAssets.filter((asset) => asset.kind === "image" || asset.kind === "carousel_slide");
