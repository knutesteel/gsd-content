...
drop function if exists public.save_content_item(uuid, integer, text, public.content_status, text, integer, text, text, text, text, numeric, integer, boolean, text, text, text);
...
begin
  ...
  update public.content_items set
    ...
    caption = nullif(p_caption, ''), first_comment = nullif(p_first_comment, ''),
    generation_prompt = nullif(p_generation_prompt, ''), score = p_score,
...
begin
  ...
  insert into public.content_items(owner_id, identifier, parent_id, variation_number, title, status, content_type,
    panel_count, overview, content, caption, first_comment, generation_prompt, score, priority, is_favorite)
  values (v_source.owner_id, v_root_identifier || '-' || v_variation, v_parent_id, v_variation,
    ...
    v_source.panel_count, v_source.overview, v_source.content, v_source.caption, v_source.first_comment, v_source.generation_prompt,
...
revoke execute on function public.save_content_item(uuid, integer, text, public.content_status, text, integer, text, text, text, text, text, numeric, integer, boolean, text, text, text) from public, anon;
grant execute on function public.save_content_item(uuid, integer, text, public.content_status, text, integer, text, text, text, text, text, numeric, integer, boolean, text, text, text) to authenticated;
